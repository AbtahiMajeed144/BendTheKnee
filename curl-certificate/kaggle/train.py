...
flags.DEFINE_string("pretrained_weights", "", help="path to pretrained weights")
flags.DEFINE_float("lambda_curl", 1e-3, help="weight for the curl penalty")
flags.DEFINE_string("data_dir", "", help="path to Kaggle dataset train dir")
# UNet
...
def train(argv):
    ...
    if FLAGS.pretrained_weights:
        print(f"Loading pretrained weights from {FLAGS.pretrained_weights}")
    ...
    with trange(FLAGS.total_steps, dynamic_ncols=True) as pbar:
        for step in pbar:
            ...
            # Curl penalty using Hutchinson trace estimator
            if FLAGS.lambda_curl > 0:
